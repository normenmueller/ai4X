...
The native delegation runtime accepts one parent-owned immutable intent and calls the provider's normal multi-agent operation directly. Only that direct result supplies child/task identity and lifecycle facts. Child prose, request fields, detached JSON, local session state, or role names cannot establish provider identity or independence. Provider permissions are recorded when exposed and otherwise exactly `not-exposed`; this never widens Collaboration-owned Assignment effects. Any emitted projection has `detachedAuthority = none`. Receipt persistence is caller-owned; the adapters and evaluator have no persistence path.

## Lifecycle triggers

- **SH-W01 — Before delegation.** Load and freshness-check the inert projection and exact profile. Observe repository free space immediately before evaluating the requested Assignment. Record the parent rollout as `unavailable` unless the provider supplies its exact identity/path pair; record the not-yet-created child as such. Host release is telemetry, never an admission allowlist.
...
- **SH-W03 — At SubagentStop.** Bind the stop to the same direct provider-returned child/task identity and one-use nonce. If the provider supplies an exact child rollout path, apply every exact-path safety check before measuring it; otherwise record `unavailable`. A read-only Assignment additionally requires before/after repository and remote nonmutation evidence. Independent review consumes exactly one verdict and then stops.
...
Stop on every evaluator `block`, stale or unknown profile, missing/ambiguous direct native result, provider identity or lifecycle mismatch, replayed nonce, second verdict, read-only mutation, incompatible exposed permissions, stale projection, incomplete admission input, foreign project root, observation outside the declared freshness bound, resource threshold crossing without durable explanation, ambiguous Assignment effects, ownership overlap, invalid exception authority, or missing durable result. Host release changes do not alone block. Unsafe exact paths fail the rollout diagnostic and every rollout claim; they do not alone grant or deny Assignment admission.
...
This policy materializes the Product Owner decisions `M121-01` and `M154-01`. Its authoritative collaboration/resource facts remain the two linked Dhall owners. `util/repository/session-hygiene/` is a Repository Engineering boundary: it may evaluate inert facts, implement the closed adapter profile, bind a direct native provider result inside the active parent boundary, and obtain exact read-only Codex metadata, but it owns neither project semantics nor detached provider truth. Host release is preserved exactly as telemetry. Layout compatibility is claimed only for an exact provider-supplied path that passes every measurement safeguard.
